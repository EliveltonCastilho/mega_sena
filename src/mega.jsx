...
const App = () => {
  ...
  const renderRealHistogram = () => {
    ...
    for (let i = startX; i <= endX; i += 3) {
      let freq = (realStats.sumFrequencies[i] || 0) + (realStats.sumFrequencies[i+1] || 0) + (realStats.sumFrequencies[i+2] || 0);
      let height = (freq / maxFreq) * 100;
      const isGoldenZone = i >= 150 && i <= 210;
      
      bars.push(
        <div 
          key={i} 
          className={`flex-1 rounded-t-sm transition-all duration-700 relative group ${isGoldenZone ? 'bg-slate-600' : 'bg-slate-800'}`}
          style={{ height: `${height * 0.4}%` }} 
        >
           <div className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[9px] p-1 rounded z-50 whitespace-nowrap">
             Soma ~{i}: {freq}x
           </div>
        </div>
      );
